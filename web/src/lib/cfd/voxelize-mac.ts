/**
 * Voxelize scene geometry into the MAC field's wall masks.
 *
 * The cell-centred wall is the canonical mask. Face masks (`uwall`,
 * `vwall`, `wwall`) are derived from it: a face is "closed" iff either
 * adjacent cell is solid, OR the face lies on the domain boundary.
 *
 * Note that this is geometry voxelization only — heat sources, AC jet
 * forcing, and infiltration flux are injected separately by sources-mac.ts.
 */

import {
  NX, NY, NZ, CK, type MACFields, type RoomDims, cellSize,
} from "./mac-grid";
import type { Obstacle, ExtensionBlock, STLObject } from "@/lib/io/schema";
import { computeRoomMask } from "@/lib/geometry/roomMask";

export function voxelizeObstaclesMAC(
    ...
    for (let iz = Math.max(0, iz0); iz < Math.min(NZ, iz1); iz++)
      for (let iy = Math.max(0, iy0); iy < Math.min(NY, iy1); iy++)
        for (let ix = Math.max(0, ix0); ix < Math.min(NX, ix1); ix++)
          f.wall[CK(ix, iy, iz)] = 1;
  }
}

export function voxelizeBlocksMAC(
  f: MACFields, room: RoomDims, blocks: ExtensionBlock[],
): void {
  const { L, W } = room;
  const { dx, dy, dz } = cellSize(room);
...
export function voxelizeSTLMAC(
  f: MACFields, room: RoomDims, stls: STLObject[],
): void {
  const { L, W } = room;
  const { dx, dy, dz } = cellSize(room);
  for (const s of stls) {
...
/**
 * Voxelize a single room-STL into f.wall using the shared roomMask
 * utility. Cells outside the L-shape (and shell cells themselves) become
 * wall=1; the L-shape interior stays fluid.
 */
function voxelizeRoomShell(
  ...
  for (let k = 0; k < NX * NY * NZ; k++) {
    if (mask[k] === 0) f.wall[k] = 1;
  }
  void cellSize; void CK;   // keep helpers in scope for the obstacle loop
}

/**
 * Derive face masks from the cell-centred wall mask. A face is closed
 * (no flow allowed) iff either adjacent cell is solid OR it lies on a
 * domain boundary. Domain boundary faces are always closed (no penetration
 * — actual openings are handled as boundary-condition source terms in
 * sources-mac.ts, not as open faces).
 */
export function buildFaceMasks(f: MACFields): void {
  // u-faces: (NX+1) × NY × NZ
  for (let iz = 0; iz < NZ; iz++)
    for (let iy = 0; iy < NY; iy++)
      for (let ix = 0; ix <= NX; ix++) {
        const ku = ix + (NX + 1) * iy + (NX + 1) * NY * iz;
        if (ix === 0 || ix === NX) { f.uwall[ku] = 1; continue; }
        const a = f.wall[CK(ix - 1, iy, iz)];
        const b = f.wall[CK(ix,     iy, iz)];
        f.uwall[ku] = (a || b) ? 1 : 0;
      }
  // v-faces: NX × (NY+1) × NZ
  for (let iz = 0; iz < NZ; iz++)
    for (let iy = 0; iy <= NY; iy++)
      for (let ix = 0; ix < NX; ix++) {
        ...
        if (iy === 0 || iy === NY) { f.vwall[kv] = 1; continue; }
        const a = f.wall[CK(ix, iy - 1, iz)];
        const b = f.wall[CK(ix, iy,     iz)];
        f.vwall[kv] = (a || b) ? 1 : 0;
      }
  // w-faces: NX × NY × (NZ+1)
  for (let iz = 0; iz <= NZ; iz++)
    for (let iy = 0; iy < NY; iy++)
      for (let ix = 0; ix < NX; ix++) {
        ...
        if (iz === 0 || iz === NZ) { f.wwall[kw] = 1; continue; }
        const a = f.wall[CK(ix, iy, iz - 1)];
        const b = f.wall[CK(ix, iy, iz    )];
        f.wwall[kw] = (a || b) ? 1 : 0;
      }
}
